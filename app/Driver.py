...
while val == True:
    try:
        ...
        theme = input(
            "Choose a theme (i.e. pirates, space adventure, etc.)? \n")
        perspective = input("What perspective, first or third? \n")
        creature = input(
            "What creature do you want to play as? (elf, bear, human, etc) \n"
        )
...
response = pe.chat.send_message(
    pe.intro_prompt(story_length, theme, perspective, creature, name)
)
print(response.text)
val = True
while val == True:
